In order to debug FuseCP.Server please:

Debugging with net48 when running FuseCP.Server on Windows:

- Debugging with IIS:
  Setup a IIS Website for FuseCP.Server pointing to the FuseCP.Server folder. The Application Pool should run with
...
- Debugging with IIS Express:
  Setup a "fusecp.server" site that can be run on iisexpress with iisexpress /site:fusecp.server 
  ...
  appcmd add site /name:fusecp.server /bindings:http/*:9018:localhost,https/*:9019:localhost /physicalPath:c:\github\fusecp\fusecp\sources\fusecp.server
