...
{if $settings.AddonsActive eq 0}
    <div id="servicecontent_addon">
        <div class="errorbox">{$LANG.FuseCP_addonsnotactive}</div>
    </div>
{else}
    ...
    function runAddonDeleteCommand(whmcs_id) {
            ...
            if(confirm('Are you sure you want to delete this Addon?')){
                ...
                var reqstr = "module=fusecp_module&action=delete_addon&id="+whmcs_id+"&ajax=1&searchAddon="+searchAddon;
...
<form class="form" role="form" id="addon_form" action="{$params['modulelink']}" method="POST">
        ...
        <div class="modal-dialog">
            <div class="modal-content panel panel-primary">
                <div class="modal-header panel-heading">
                    ...
                    <h4 class="modal-title" id="modal_addon_edit_form_Label">{$LANG.FuseCP_edit_addon_option}</h4>
                </div>
                ...
                <div class="modal-footer panel-footer">
                    <input type="submit" onclick="runAddonSaveCommand();return false;" value="{$LANG.FuseCP_save_changes}" class="btn btn-primary">
	...
	<div class="panel-heading clearfix" style="background-color: lightgray;">
		<div class="form-group">
			<input type="text" id="searchAddon" name="searchAddon" value="{$searchAddon}" onkeyup="addonSearch(0, 2)" onclick="addonSearch(0, 2)" placeholder="{$LANG.FuseCP_search_addon}" style="width: 500px;"/>
		</div>
		<div class="form-group">
			<label><input type="checkbox" id="showOnlyAssignedAddon" name="showOnlyAssignedAddon" value="1" {if $showOnlyAssignedAddon}checked{/if} onclick="addonSearch(0, 2)" /> {$LANG.FuseCP_show_only_assigned_addons}</label>
		</div>
	</div>
